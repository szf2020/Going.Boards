...
using Going.Boards.Chips.I2C;
using Mono.Unix.Native;
using Mono.Unix;

namespace Going.Boards.Chips.I2C.I2CPI
{
    public class I2CBusPI : IDisposable
    {
        public const string DefaultDeviceFileFormat = "/dev/i2c-1";
        ...
        public I2CBusPI(int index)
        {
            _device = string.Format(DefaultDeviceFileFormat, index);
        }

        public I2CBusPI(int index, string deviceFileFormat)
        {
            _device = string.Format(deviceFileFormat, index);
        }

        public I2CBusPI(string deviceFile)
        {
            _device = deviceFile;
        }
